...
class UserService {
  ...
  createUserWithGoogleAuth(options) {
    return this.userModel.createUserWithGoogleAuth(options).then(() => {
      return this.getUserbyEmail(options.email);
    });
  }
  createUserWithFacebookAuth(options) {
    return this.userModel.createUserWithFacebookAuth(options).then(() => {
      return this.getUserbyEmail(options.email);
    });
  }
  ...
  createFullNameIdArray(users) {
    return users.map(user => {
      return {
        id: user.getUserId(),
        fullname: user.getUserFirstName() + " " + user.getUserLastName()
      };
    });
  }
}
