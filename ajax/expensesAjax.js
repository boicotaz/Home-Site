    ...
    expenses.forEach(expense => {
      if (Array.isArray(expense)) {
        expense.forEach(transaction => {
          transaction.creditorFullName =
            usersInGroupDetails.get(parseInt(transaction.creditor, 10))
              .firstName +
            " " +
            usersInGroupDetails.get(parseInt(transaction.creditor, 10))
              .lastName;
          transaction.debtorFullName =
            usersInGroupDetails.get(parseInt(transaction.debtor, 10))
              .firstName +
            " " +
            usersInGroupDetails.get(parseInt(transaction.debtor, 10)).lastName;
        });
      } else {
        expense.creditorFullName =
          usersInGroupDetails.get(parseInt(expense.creditor, 10)).firstName +
          " " +
          usersInGroupDetails.get(parseInt(expense.creditor, 10)).lastName;
        expense.debtorFullName =
          usersInGroupDetails.get(parseInt(expense.debtor, 10)).firstName +
          " " +
          usersInGroupDetails.get(parseInt(expense.debtor, 10)).lastName;
      }
    });
  } else {
    expenses.creditorFullName =
      usersInGroupDetails.get(parseInt(expenses.creditor, 10)).firstName +
      " " +
      usersInGroupDetails.get(parseInt(expenses.creditor, 10)).lastName;
    expenses.debtorFullName =
      usersInGroupDetails.get(parseInt(expenses.debtor, 10)).firstName +
      " " +
      usersInGroupDetails.get(parseInt(expenses.debtor, 10)).lastName;
  }
