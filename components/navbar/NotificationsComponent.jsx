export default class Notifications extends React.Component {
  constructor(props) {
    ...
    this.state.notifications.dataArray = this.props.notifications.filter(
      notification => {
    ...
    console.log(
      "Notifications in contructor__________________________",
      this.props.notifications
    );
    console.log(
      "NOTIFICATIONS CALLED________________________-----___##############################",
      this.state
    );
  }
  ...
  componentWillReceiveProps(nextProps) {
    console.log(
      "Notifications in will receive props__________________________",
      nextProps
    );
    // if (nextProps.notifications[nextProps.notifications.length - 1].userId == this.state.currentUser.id) return;
    let updatedNotifications = {};
    updatedNotifications.dataArray = nextProps.notifications.filter(
      notification => {
        if (this.state.currentUser.id != notification.userId)
          return notification;
      }
    ...
    updatedNotifications.newNotificationsSum =
      updatedNotifications.dataArray.length;
    if (
      updatedNotifications.newNotificationsSum >
      this.state.notifications.newNotificationsSum
    ) {
    ...
    this.setState({
      ...
      usersInGroup: nextProps.usersInGroup
    });
  }
  ...
  constructProperties(notification) {
    if (notification.userId == this.state.currentUser.id) return null;
    ...
    if (notificationCreator.profImgExists) {
      notificationProperties.imgPath =
        "public/uploads/profImg_user" + notificationCreator.userId + "_.png";
    } else {
    ...
    notificationProperties.createdAt = formatDate(
  ...
  render() {
    console.log(
      "NOTIFICATIONS STATE IS________________________-----___##############################",
      this.state
    );
    ...
    let notificationsItems = this.state.notifications.dataArray
      .map(notification => {
        ...
        return (
          <React.Fragment>
            <div className="dropdown-item p-0 pt-3 pb-3 h-100 dropdown-item--custom-width dropdown-item--shadow">
              <div
                className="row m-1 p-0 ml-3 align-content-center"
                style={{ maxHeight: "8rem", overflowY: "auto", height: "8rem" }}
              >
                <div className="col-2 ml-0 pl-0 pr-0">
                  <img
                    src={notificationProperties.imgPath}
                    className="img-thumbnail rounded-circle img-pos"
                    alt="UserImg"
                  />
                  <i
                    className={
                      "fas " + notificationProperties.badge + " fa-custom"
                    }
                  />
                </div>
                <div className="col-10 p-0">
                  <p
                    className="text-light rounded m-0"
                    style={{
                      width: "100%",
                  ...
                  >
                    {" "}
                    <b> {notificationProperties.FirstName} </b>{" "}
                    {" " + notificationProperties.action + " a new "}{" "}
                    <b> {notificationProperties.type} </b>
                  </p>
                  <p className="text-light rounded m-0 notification-text">
                    {notificationProperties.createdAt}
                  </p>
                </div>
              </div>
      ...
      <React.Fragment>
        ...
        >
          <div className="fa-container">
            ...
            />
            <i
              className="badge badge-pill badge-danger badge--font"
              style={newNotificationsStyle}
            >
              {this.state.notifications.newNotificationsSum}
            </i>
          </div>
          <div
            className="dropdown-menu dropdown-menu-right p-0 dropdown-menu--rounded dropdown-menu--custom bg-dark"
            aria-labelledby="navbarDropdownMenuLink"
          >
            <span className="dropdown-header dropdown-header--font dropdown-header--grey ">
              Notifications
            </span>
            {notificationsItems}
          </div>
