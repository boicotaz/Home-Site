...
expensesController.post("/create-expense", authValidation, function(
  req,
  res,
  next
) {
  let { creditor, debtors, info } = req.body;
  groupService.findGroupByUserId(creditor.id).then(group =>
...
expensesController.get("/get-expense-table", authValidation, async function(
  req,
  res,
  next
) {
  groupService.findGroupByUserId(req.user.id).then(group => {
    getGroupExpenseTableData(group.getGroupId()).then(expensesData =>
      res.json(expensesData)
    );
  });
});

expensesController.get("/get-expense-totals-table", authValidation, function(
  req,
  res,
  next
) {
  groupService.findGroupByUserId(req.user.id).then(group => {
    let getUsersInGroupPromise = groupService.findUsersInGroup(
      group.getGroupId()
    );
    let getGroupExpenseTableDataPromise = getGroupExpenseTableData(
      group.getGroupId()
    );

    Promise.all([
      getUsersInGroupPromise,
      getGroupExpenseTableDataPromise
    ]).then(data => {
      ...
      for (let i = 0; i < fullNames.length; i++) {
        totalExpenses[fullNames[i].id] = {
          fullname: fullNames[i].fullname,
          debtSum: 0
        };
        debts = {};
      ...
      groupExpenses.map(expenseEntry => {
        ...
        expenseEntry.map(transaction => {
          ...
          if (totalExpenses[creditorId].debts[debtorId] > 0) {
            // does tolis owes any money to maria, from a past expense?
            if (transaction.debt <= totalExpenses[creditorId].debts[debtorId]) {
              // is the debt enough to settle the current transaction ?
              totalExpenses[creditorId].debts[debtorId] -= transaction.debt;
              totalExpenses[debtorId].debts[creditorId] += transaction.debt;
              return;
            } else {
              // settle up some of the debt
              remainingDebt =
                transaction.debt - totalExpenses[creditorId].debts[debtorId];
            ...
            // Maria doesnt owe money to tolis
          ...
          while (remainingDebt > 0 && CommonDebtsFlag) {
            ...
            Object.keys(totalExpenses[creditorId]).forEach((key, index) => {
              ...
              if (!key == debtorId) {
                if (totalExpenses[key].debts[creditorId] < 0) {
                  // user identified by "key" owes some money to our creditor
                  if (totalExpenses[key].debts[debtorId] > 0) {
                    //user with key to be of any use, he should at least owe some money to the debtor
                    let userDebtValue = 1;
                    if (
                      totalExpenses[key].debts[creditorId] <
                      -1 * remainingDebt
                    )
                      userDebtValue = 2;
                    if (
                      totalExpenses[key].debts[creditorId] <
                        -1 * remainingDebt &&
                      totalExpenses[key].debts[debtorId] > remainingDebt
                    )
                      userDebtValue = 3;
            ...
            switch (searchResult.weight) {
              ...
              case 1:
                ...
                remainingDebt -= Math.abs(
                  totalExpenses[searchResult.userId].debts[creditorId]
                );
                ...
                totalExpenses[creditorId].debts[
                  searchResult.userId
                ] -= paidDebt;
                totalExpenses[searchResult.userId].debts[
                  creditorId
                ] += paidDebt;
      ...
      Object.keys(totalExpenses).forEach((key, index) => {
        Object.keys(
          totalExpenses[key].debts
        ).forEach((innerKey, innerIndex) => {
...
async function getGroupExpenseTableData(groupId) {
  return expenseService
    .findAllExpensesByGroupId(group.getGroupId())
    .then(expenses => {
      var expensesData = expenses.map(expense => {
        return transactionService
          .findAllTransactionsByExpenseId(expense.getExpenseId())
          .then(transactions => {
            var trans_data = transactions.map(transaction => {
              return {
                creditor: transaction.getCreditor(),
                debtor: transaction.getDebtor(),
                when: expense.getDate(),
                description: expense.getDescription(),
                credit: expense.getSum(),
