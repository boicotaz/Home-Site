  ...
  if (req.cookies["auth_logged"] != undefined && req.isAuthenticated()) {
    if (req.cookies["auth_logged"] == "yes") {
      if (req.cookies["remember_me"] != undefined) {
        sessionService
          .findSessionBySessionId(req.cookies["remember_me"])
          .then(session => {
            if (Array.isArray(session) && session.length) {
              ...
              if (userId == req.user.id) {
                if (req.cookies["remember_me"] != req.sessionID) {
                  ...
                  res.cookie("remember_me", req.sessionID, {
                    path: "/",
                    httpOnly: true,
                    maxAge: 604800000,
                    secure: true
                  }); // TO DO: sortain the age
    ...
    if (req.cookies["auth_logged"] != undefined) res.clearCookie("auth_logged");
...
logInController.get(
  ...
  (req, res, next) => {
    if (req.query.remember_me == "yes") {
      res.cookie("login_remember_me", "yes", {
...
logInController.get(
  ...
  (req, res) => {
    ...
    res.cookie("auth_logged", remember_me, {
      path: "/",
      httpOnly: true,
      maxAge: 604800000,
      secure: true
...
logInController.post("/validate", function(req, res, next) {
  passport.authenticate("local", function(err, user, info) {
    if (err) {
      ...
      return res.render("sign-in-room8s.ejs", {
        user: false,
        userAlreadyExists: null,
        mail: null
      });
