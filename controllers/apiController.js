...
apiController.post("/get-users-in-group", function(req, res) {
  ...
  groupService.findGroupByUserId(user.id).then(group => {
    groupService.findUsersInGroup(group.getGroupId()).then(users => {
      ...
      users.forEach(user => {
        groupUsersDetails.set(user.id, {
          firstName: user.firstName,
          lastName: user.lastName,
          profImgExists: user.profImgExists,
          userId: user.id
        });
      });
...
apiController.get("/get-group-details", function(req, res) {
  console.log(
    "_______________________request for group details by user with id: ",
    req.user.id,
    "___________________________"
  );
  groupService
    .findGroupByUserId(req.user.id)
    .then(group =>
      res.json({ groupName: group.getGroupName(), groupId: group.getGroupId() })
    );
});
