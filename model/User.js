...
UserModel.prototype.comparePasswords = function comparePasswords(
  password,
  callback
) {
...
UserModel.getUserByName = async function(name) {
  return this.findOne({
    where: { firstName: name[0], lastName: name[1] },
...
UserModel.getAllUsers = async function() {
  return await UserModel.findAll({
    ...
    let userNamesAndIds = users.map(user => {
